# copy from https://github.com/RemotePixel/amazonlinux-gdal/blob/gdal2.4.2/Dockerfile
# only updating python version, but keep everything else as is
# Gdal 3.x has performance issues due to changes in Proj.
# We will continue to work with GDAL 2.4 for now in our lambda functions
...
RUN cd /tmp/gdal \
  && touch config.rpath \
  && CPPFLAGS="-I$PREFIX/include" LDFLAGS="-Wl,-rpath,$PREFIX/lib -Wl,-rpath,$PREFIX/lib64 -Wl,-z,origin" CFLAGS="-O2 -Wl,-S" CXXFLAGS="-O2 -Wl,-S" ./configure \
      --prefix=$PREFIX \
      --with-proj=$PREFIX \
